...
def detect_circles(image, min_radius, max_radius, threshold):
    ...
    circles = cv.HoughCircles(image, cv.HOUGH_GRADIENT, 1, 12, param1=50 , param2=threshold,
                              minRadius=min_radius, maxRadius=max_radius) #1,12,50
...
# blur the gray image
blur = cv.GaussianBlur(gray, (11, 11), sigmaX = 20, sigmaY= 20)
cv.imshow('blur',blur)

# adaptive thresholding
ad_th = cv.adaptiveThreshold(blur, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 51, 9) #255, 47, 4
cv.imshow('Threshold',ad_th)

# canny edge detector
canny = cv.Canny(ad_th, 125, 255) #0,50
